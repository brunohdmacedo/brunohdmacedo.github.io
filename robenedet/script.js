 ...
 * Simulações Físicas, Internacionalização (i18n), Alternador Dark/Light e Efeitos Dinâmicos
 */
...
// 1. Alternador de Tema Escuro / Claro (Dark & Light Mode)
function initThemeToggle() {
  const themeToggle = document.getElementById('themeToggle');
  const themeIcon = document.getElementById('themeIcon');
  const root = document.documentElement;

  // Carregar preferência salva ou usar escuro por padrão
  const savedTheme = localStorage.getItem('renata_theme') || 'dark';
  setTheme(savedTheme);
  ...
  function setTheme(theme) {
    ...
    localStorage.setItem('renata_theme', theme);
...
// 4. Efeito Giroscópico / Tilt no Card do Emblema
...
// 5. Fundo Dinâmico de Partículas de Raios X (Canvas)
function initBackgroundCanvas() {
  ...
  function animate() {
    ...
    ctx.strokeStyle = isLight ? 'rgba(147, 51, 234, 0.08)' : 'rgba(216, 180, 254, 0.04)';
    ctx.lineWidth = 1.5;
    ctx.beginPath();
...
// 6. Simulador Físico de Bancada (CS-XSI Detector)
...
// 7. Busca e Filtragem de Publicações em Tempo Real
...
// 8. Copiador de Citações com Feedback Toast
